...
export type ResourceTypeId = 
  ...
  | "resourcepack" | "shader" | "building" | "audio"
  | "ecoTool" | "doc" | "community" | "image";

const allResourceTypes: { id: ResourceTypeId; label: string; icon: React.ReactNode; group: string }[] = [
  { id: "mod", label: "模组", icon: <ModIcon className="w-4 h-4" />, group: "游戏内容" },
  { id: "map", label: "地图", icon: <MapIcon className="w-4 h-4" />, group: "游戏内容" },
  { id: "datapack", label: "数据包", icon: <DataPackIcon className="w-4 h-4" />, group: "游戏内容" },
  { id: "modpack", label: "整合包", icon: <ModpackIcon className="w-4 h-4" />, group: "游戏内容" },
  { id: "server", label: "服务器", icon: <ServerIcon className="w-4 h-4" />, group: "游戏内容" },
  { id: "resourcepack", label: "材质", icon: <ResourcePackIcon className="w-4 h-4" />, group: "辅助资源" },
  { id: "shader", label: "光影", icon: <ShaderIcon className="w-4 h-4" />, group: "辅助资源" },
  { id: "building", label: "建筑", icon: <BuildingIcon className="w-4 h-4" />, group: "辅助资源" },
  { id: "audio", label: "音频", icon: <AudioIcon className="w-4 h-4" />, group: "辅助资源" },
  { id: "ecoTool", label: "生态工具", icon: <ToolIcon className="w-4 h-4" />, group: "创作生态" },
  { id: "doc", label: "知识文档", icon: <DocIcon className="w-4 h-4" />, group: "创作生态" },
  { id: "community", label: "社交社区", icon: <UserIcon className="w-4 h-4" />, group: "创作生态" },
  { id: "image", label: "形象", icon: <SkinIcon className="w-4 h-4" />, group: "形象" },
];

// Content/auxiliary resource IDs that use common meta
const contentResourceIds = ["mod", "map", "datapack"];
const modpackServerResourceIds = ["modpack", "server"];
const auxiliaryResourceIds = ["resourcepack", "shader", "building", "audio"];

interface FilterPanelProps {
  resourceId: ResourceTypeId;
  selectedTags: Record<string, string[]>;
  onTagsChange: (tags: Record<string, string[]>) => void;
  onResourceChange: (id: ResourceTypeId) => void;
}
...
const FilterPanel = ({
  ...
  const isContentResource = contentResourceIds.includes(resourceId);
  const isModpackServerResource = modpackServerResourceIds.includes(resourceId);
  const isAuxiliaryResource = auxiliaryResourceIds.includes(resourceId);

  const allTags: MainTag[] = tagSystem
    ? [
        ...tagSystem.mainTags,
        ...(isContentResource ? contentCommonMeta : []),
        ...(isModpackServerResource ? modpackServerCommonMeta : []),
  ...
  const handleResourceSwitch = (id: ResourceTypeId) => {
    onResourceChange(id);
    onTagsChange({});
  };

  // Group resource types by category for the switcher
  const resourceGroups = allResourceTypes.reduce((acc, type) => {
    if (!acc[type.group]) acc[type.group] = [];
    acc[type.group].push(type);
    return acc;
  }, {} as Record<string, typeof allResourceTypes>);
    ...
    <>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          {/* Resource type switcher grouped */}
          <div className="border-b border-border pb-3 mb-3">
            <div className="text-xs text-muted-foreground mb-2">资源类型</div>
            <div className="space-y-2">
              {Object.entries(resourceGroups).map(([groupName, types]) => (
                <div key={groupName}>
                  <div className="text-[10px] text-muted-foreground/60 mb-1">{groupName}</div>
                  <div className="flex flex-wrap gap-1.5">
                    {types.map((type) => (
                      <button
                        key={type.id}
